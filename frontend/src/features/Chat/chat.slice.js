...
const chatSlice = createSlice({
    ...
    reducers: {
        ...
        replaceChatId: (state, action) => {
            const { oldId, newId, chatData: newChatData } = action.payload;
            const chatData = state.chats[oldId];
            if (chatData) {
                chatData.id = newId;
                if (newChatData && newChatData.title) chatData.title = newChatData.title;
...
export const { setChats, setCurrentChatId, setLoading, setError, createNewChat, addNewMessage, addMessages, replaceChatId } = chatSlice.actions
